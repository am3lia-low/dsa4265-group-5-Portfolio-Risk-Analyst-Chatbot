...
------
  ...
  kb5_strategies.txt                          — human-readable audit
...
STRATEGIES_DIR  = "knowledge_base/strategies"
STRATEGIES_FILE = os.path.join(STRATEGIES_DIR, "strategies.json")
OUTPUT_TXT      = "output/kb5_strategies.txt"
...
class StrategyStore:
    ...
    def export_txt(self, output_path: str = OUTPUT_TXT) -> str:
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            f.write(f"# kb5 STRATEGIES — {len(chunks)} frameworks — generated {now}\n\n")
            for c in chunks:
                f.write(f"--- {c['strategy_key']} [{c['category']}] ---\n")
                f.write(f"Label: {c['label']}\n")
                f.write(c["text"] + "\n\n")
 
        print(f"  ✓ kb5 TXT → {output_path}  ({len(chunks)} strategies)")
        return output_path
 
    def save_json(self) -> str:
        with open(STRATEGIES_FILE, "w", encoding="utf-8") as f:
            json.dump(STRATEGIES, f, indent=2)
        print(f"  ✓ kb5 JSON → {STRATEGIES_FILE}")
